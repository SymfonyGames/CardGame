...
public class CardPortal : Card
{
    ...
    #region Init
    public override void Init(CardDataPortal data, LevelThemeData theme)
    {
    ...
    void InitImages(CardDataPortal data, LevelThemeData theme)
    {
        art.sprite = data.Art;
        if(!theme)return;
    ...
    void InitStats(CardDataPortal data)
    {
        _data = data;
    }
    void InitComponents()
    ...
    public override void Interact(Card card, int iteration)
    {
        ...
        EventManager.Instance.PortalOpen(_data);
        Dead();
    }
